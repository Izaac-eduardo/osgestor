...
const initial = (entrada: Entrada | null, produtos: AbastecimentoProduto[]): Form => ({
  ...
  litros_nf: entrada ? String(entrada.litros_nf) : '', valor_total_nf: entrada ? String(entrada.valor_total_nf) : '',
...
export function EntradaFormModal({ entrada, produtos, pontos, compatibilidades, saving, error, onClose, onSave }: Props) {
  ...
  const total = Object.values(form.destinos).reduce((sum, value) => sum + (Number(value) || 0), 0)
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!(Number(form.litros_nf) > 0)) next.litros_nf = 'Informe litros maiores que zero.'
    if (Number(form.valor_total_nf) < 0 || form.valor_total_nf === '') next.valor_total_nf = 'Informe um valor igual ou maior que zero.'
    for (const value of Object.values(form.destinos)) if (value && !(Number(value) > 0)) next.destinos = 'Cada destino preenchido deve ter litros maiores que zero.'
    setErrors(next)
    if (Object.keys(next).length) return
    onSave({ data_entrada: form.data_entrada, numero_nf: form.numero_nf.trim(), produto_id: form.produto_id, litros_nf: Number(form.litros_nf), valor_total_nf: Number(form.valor_total_nf), observacoes: form.observacoes.trim() || null, destinos: Object.entries(form.destinos).filter(([, value]) => value.trim() !== '').map(([ponto_id, litros]) => ({ ponto_id, litros: Number(litros) })) })
  }
  ...
  return <div className="modal-backdrop"><form className="order-form abastecimento-form" role="dialog" aria-modal="true" aria-labelledby="entrada-form-title" onSubmit={submit}>
    ...
    <main><div className="form-grid"><Field label="Data" error={errors.data_entrada}><input type="date" value={form.data_entrada} onChange={event => set('data_entrada', event.target.value)} /></Field><Field label="Número da NF" error={errors.numero_nf}><input value={form.numero_nf} maxLength={80} onChange={event => set('numero_nf', event.target.value)} /></Field><Field label="Produto" error={errors.produto_id}><select value={form.produto_id} onChange={event => set('produto_id', event.target.value)}>{produtos.map(item => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></Field><Field label="Litros da NF" error={errors.litros_nf}><input type="number" min="0.001" step="0.001" value={form.litros_nf} onChange={event => set('litros_nf', event.target.value)} /></Field><Field label="Valor total da NF" error={errors.valor_total_nf}><input type="number" min="0" step="0.01" value={form.valor_total_nf} onChange={event => set('valor_total_nf', event.target.value)} /></Field><label className="form-field form-field--wide">Observações<textarea rows={3} value={form.observacoes} onChange={event => set('observacoes', event.target.value)} /></label></div>
      {produto?.permite_distribuicao && <section className="abastecimento-distribution"><div className="abastecimento-section-heading"><div><h3>Distribuição</h3><p>Informe somente os pontos que receberam litros.</p></div><strong>Total distribuído: {formatQuantity(total)} L</strong></div>{pontosCompativeis.length ? <div className="distribution-grid"><div className="distribution-grid__header"><span>Ponto</span><span>Litros</span></div>{pontosCompativeis.map(ponto => <label className="distribution-row" key={ponto.id}><span><strong>{ponto.codigo}</strong><small>{ponto.nome}</small></span><input aria-label={`Litros para ${ponto.codigo}`} type="number" min="0" step="0.001" value={form.destinos[ponto.id] || ''} onChange={event => setForm(current => ({ ...current, destinos: { ...current.destinos, [ponto.id]: event.target.value } }))} /></label>)}</div> : <p className="abastecimento-muted">Nenhum ponto compatível ativo para este produto.</p>}{errors.destinos && <p className="form-api-error" role="alert">{errors.destinos}</p>}</section>}
