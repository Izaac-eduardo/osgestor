...
routes.get('/health/database', async (_request, response) => {
  try {
    ...
    response.status(200).json({ status: 'ok', message: 'Banco de dados conectado.', databaseTime: result.rows[0]?.now ?? null });
  } catch (error) {
    console.error('Falha na verificação da conexão com o banco de dados.', error);
    response.status(500).json({ status: 'error', message: 'Não foi possível conectar ao banco de dados.' });
  }
});
