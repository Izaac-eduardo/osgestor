...
export async function getRelatorioEntradas(filters: EntradasRelatorioFilters = {}) {
  ...
  const page = Math.max(1, Number(filters.page ?? 1) || 1); const limit = unlimited ? 0 : Math.min(100, Math.max(1, Number(filters.limit ?? 25) || 25));
  ...
  const offset = (page - 1) * limit; const pageValues = [...values, String(limit), String(offset)]; const rowsQuery = `SELECT e.id,e.data_entrada,e.numero_nf,e.produto_id,p.codigo produto_codigo,p.nome produto_nome,e.litros_nf,e.valor_total_nf ${base} ${clause} ORDER BY e.data_entrada DESC,e.created_at DESC,e.id DESC${unlimited ? '' : ` LIMIT $${pageValues.length - 1} OFFSET $${pageValues.length}`}`; const rows = (await pool.query(rowsQuery, unlimited ? values : pageValues)).rows as Array<Record<string, unknown>>;
