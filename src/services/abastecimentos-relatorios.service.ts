...
export interface RelatorioAbastecimentosFilters { data_inicio?: string; data_fim?: string; obra_id?: string; busca?: string; produto?: string; tipo_destinatario?: string }
...
export async function getRelatorioAbastecimentos(filters: RelatorioAbastecimentosFilters = {}) {
  ...
  const [summary, products, works, fleets, thirds, specials, evolution] = await Promise.all([
    ...
    query(`SELECT to_char(a.data_hora::date,'YYYY-MM-DD') data,COUNT(*)::text quantidade,COALESCE(SUM(a.litros),0)::text litros,COALESCE(SUM(a.valor_total),0)::text valor ${from} ${clause} GROUP BY a.data_hora::date ORDER BY a.data_hora::date`),
  ]);
  return { summary: numbers(summary.rows[0] || { quantidade: '0', litros: '0', valor: '0', destinatarios: '0' }), por_produto: products.rows.map(numbers), por_obra: works.rows.map(numbers), por_frota: fleets.rows.map(numbers), por_terceiro: thirds.rows.map(numbers), especiais: specials.rows.map(numbers), evolucao: evolution.rows.map(numbers) };
}
