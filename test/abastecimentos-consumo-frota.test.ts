...
test('relatório de consumo calcula intervalos auditáveis, filtros e qualidade sem misturar produtos', async () => {
  ...
  try {
    ...
    const weighted = await get(`frota_id=${catalog.secondFleet.id}&produto=DIESEL_S500`); assert.equal(weighted.frotas[0].km_total, 300); assert.equal(weighted.frotas[0].litros_considerados, 115); assert.ok(Math.abs(weighted.frotas[0].media_km_l - (300 / 115)) < 0.000000001); assert.equal(weighted.frotas[0].intervalos_validos, 2); assert.equal(weighted.frotas[0].intervalos[0].numero_intervalo, null); assert.equal(weighted.frotas[0].intervalos[1].numero_intervalo, 1); assert.equal(weighted.frotas[0].intervalos[2].numero_intervalo, 2); assert.equal(weighted.frotas[0].intervalos.filter((item: any) => item.status === 'VALIDO')[1].abastecimentos.length, 2);
...
test('relatório de consumo preserva obra, paginação e leituras ausentes sem criar intervalos artificiais', async () => {
  ...
  try {
    ...
    assert.equal(regression.frotas[0].situacao, 'PROBLEMATICA'); assert.equal(regression.frotas[0].intervalos_validos, 0); assert.equal(regression.frotas[0].media, null); assert.equal(regression.frotas[0].intervalos[0].numero_intervalo, null); assert.equal(regression.frotas[0].intervalos[1].numero_intervalo, 1); assert.equal(regression.frotas[0].intervalos[1].status, 'LEITURA_REGRESSIVA');
